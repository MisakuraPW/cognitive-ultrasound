...
> 此文启动命令对应已经运行的 v1 包。新的加速默认入口、硬件实测选择与适用范围见 [默认加速与运行前检查](accelerated_experiments.md)。旧结果无需重新跑，新包默认输出目录为 `preparation_auto_v2`。
...
## 2026-09-19 资源诊断后的可选图执行

服务器实测原版小规模 CASL 热态约 0.078 秒/帧，25 步混合精度约 0.032 秒/帧；独立确认集加速约 2.53 倍。进程启动、权重加载、首次图编译、指标和保存期间 GPU 不必持续满载。

BF eager 训练则出现约 1 个 CPU 核忙碌、GPU 利用率 0–2% 的情况。针对 Python 逐算子调度，新增 `configs/preparation_graph.yaml`：codec、prior 和固定 uniform 策略的 filter 以 `tf.function` 执行训练更新。保留 FP32、原批大小、采样顺序、损失、Adam 状态、梯度裁剪与帧间 stop-gradient。固定动作下不参与损失的 NumPy 选线诊断不再每步拷回 CPU；greedy 策略没有被替换，且不能误用这条 filter 图路径。

图模式在一个训练阶段内保持同一模型和执行图，不每 50 步重建模型；仍按原间隔保存 checkpoint 和做验证。数值一致性与优化器恢复有 CPU 自动测试；GPU 实际加速比尚未测定。图编译也有一次性成本，小于几十步的任务未必受益。

这是未来新运行的显式选项，当前运行仍使用原代码。不能改正在运行的配置或覆盖其源码后，强行绕过 identity 检查续跑。若之后采用新配置，使用新的输出目录并分别报告冷启动成本、热态步耗时和端到端时间。增加样本量、粒子数或 batch 来填满 GPU 会改变实验设定，不能当作纯实现优化；同时跑计时对照也会污染速度比较。
