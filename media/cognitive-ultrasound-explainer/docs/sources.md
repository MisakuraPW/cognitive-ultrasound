...
2. 本仓库 docs/CASL_architecture.md；vendor/casl/ulsa/agent.py 的 recover（约 610–659 行）：measurement_buffer → posterior_sample → 当前帧 posterior → action_selection → 下一 mask 写入状态。画面严格跨帧更新位置。vendor/casl/ulsa/selection.py 只用来核对策略接口；不把其他策略当 CASL 主策略。官方主策略是基于后验样本的熵与贪心重权，不是简单方差 top-k。v2 以局部热区表示重建不确定性；热区与候选线的关联是定性解释，没有复现数学策略。
...
- observed-* 是教学场景的合成观测素材；estimate-* 是分别扰动的示意重建。已测方向覆盖在估计之上。possible-* 是旧版几种可能形态的档案，v2 不展示；它们不是 CASL 后验输出。热图是手工布置的解释图，不是计算得到的熵或真实误差。
...
## 科学边界
全片限定传统逐线聚焦扫描的简化解释。线密度是空间采样程度，不能直接等同真实空间分辨率。影片固定 30 fps 与超声采集更新率不同。v2 取舍画面使用 24 / 9 条线，并同时改变范围和深度；它是定性总览，不是控制变量实验。84 / 25 个视频帧的更新周期仅供观察持帧动作，无实际 FPS 或提速倍数。下一帧示意始终 6 条，不宣称每帧固定线加额外线。速度收益仍取决于计算和设备实现。
