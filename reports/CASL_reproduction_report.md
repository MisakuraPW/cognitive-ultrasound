...
已下载并核对官方患者划分：6985 train / 500 val / 500 test，极坐标输出应为 `(T,112,112)`，范围 `[-60,0]`。当前没有 EchoNet 医学视频；`dataset_statistics.md` 中 available=0 是真实状态。
...
1. 取得具有访问权限的 EchoNet 原始数据，运行转换与完整统计核验。
...
最终自动化测试：22 passed。代码静态检查通过，`pip check` 未发现依赖声明冲突。另已执行上文所述的真实模型合成集成测试；这些检查的通过不代替真实数据复现。
